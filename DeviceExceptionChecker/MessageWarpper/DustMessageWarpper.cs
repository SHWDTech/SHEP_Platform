    ...
    public class DustMessageWarpper
    {
        public static StringBuilder WarpperMessage(DeviceExceptionType type, string devName, string statName, object data = null)
        {
            switch (type)
            {
                ...
                case DeviceExceptionType.NoiseNightExcessive:
                    return NoiseExcessiveExceptionMessage(devName, statName, data);
                case DeviceExceptionType.TspExcessive:
                    return TspDataExceptionMessage(devName, statName, data);
                default:
                    return null;
            }
        ...
        private static StringBuilder NoiseExcessiveExceptionMessage(string devName, string statName, object data)
        {
            var builder = new StringBuilder($"\r\n【***扬尘设备噪音异常提醒***】\r\n扬尘设备噪音数据异常，工地名：{statName}，设备名：{devName}，异常值：{data}，异常时间：{DateTime.Now:yyyy-MM-dd HH:mm:ss}\r\n【上海卫东】");
            return builder;
        }
    }
}
